...
console.log("File calendario_new.js caricato correttamente!");
    ...
    sampleArtifacts.forEach((art) => {
      ...
      card.innerHTML = `
        <img src="${art.image}" alt="${art.name}" 
             ...
             style="cursor: pointer; width: 60px; height: 60px; border-radius: 8px; object-fit: cover; border: 2px solid rgba(100, 255, 218, 0.3);"
             onclick="openMaterialImagePopup('${art.image}', '${
        art.name
      }', '${artifactDesc.replace(/'/g, "\\'")}')">
        <div class="artifact-info">
          <div class="artifact-name" style="font-weight: 600; color: #e0e0e0; font-size: 0.9rem; margin-bottom: 0.3rem;">${
            art.name
          }</div>
          <div style="margin: 0.3em 0 0.5em 0;">
            <span style="color:#64ffda;font-size:0.85em;cursor:pointer;" onclick="event.stopPropagation(); openMaterialImagePopup('${
              art.image
            }', '${art.name}', '${artifactDesc.replace(
        /'/g,
        "\\'"
...
function renderSelectedArtifactToCreateDisplay() {
  ...
  if (selectedArtifactToCreate) {
    ...
    display.innerHTML = `
      <img src="${selectedArtifactToCreate.image}" alt="${
      selectedArtifactToCreate.name
    }" 
            onerror="this.src='images/characters/placeholder.svg'"
            onclick="openMaterialImagePopup('${
              selectedArtifactToCreate.image
            }', '${selectedArtifactToCreate.name}', '${artifactDesc.replace(
      /'/g,
      "\\'"
    )}')"
            title="Clicca per vedere la descrizione e l'immagine in grande"
            style="cursor: pointer; width: 50px; height: 50px; border-radius: 8px; object-fit: cover; border: 2px solid rgba(100, 255, 218, 0.5);">
      <span style="color: #64ffda; font-weight: 600; font-size: 1.1rem;">${
        selectedArtifactToCreate.name
      }</span>
      <button onclick="removeSelectedArtifactToCreate()" style="
        ...
        transition: all 0.3s ease;
        margin-left: auto;
      " onmouseover="this.style.background='rgba(255, 107, 107, 0.3)'" onmouseout="this.style.background='rgba(255, 107, 107, 0.2)'">×</button>
    `;
  } else {
    display.innerHTML =
...
function selectArtifactToCreate(artifact) {
  ...
  if (container) {
    ...
    container.innerHTML = `<img src="${artifact.image}" alt="${
      artifact.name
      ...
      artifact.image
    }', '${artifact.name}', '${artifactDesc.replace(/'/g, "\\'")}')">`;
  }
...
function confirmArtifactToCreateSelection() {
  if (selectedArtifactToCreate) {
    showNotification(
      `Manufatto "${selectedArtifactToCreate.name}" selezionato per la creazione! ⚒️`
    );
...
document.addEventListener("DOMContentLoaded", function () {
  console.log("Calendario nuovo inizializzato!");
});
